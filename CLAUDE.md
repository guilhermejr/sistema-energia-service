...
Solar energy generation and consumption tracking — readings, bills and historical series.

| | |
|---|---|
| Port | `9001` |
| Context path | `/energia-service` |
| Role required | `ROLE_ENERGIA` |

Part of a personal microservices system; sibling repos live at `../sistema-*`. The API gateway fronts it at `https://sistema-backend.guilhermejr.net/energia-service`.
...
The only service here with a real test suite: `@DataJpaTest` repositories, `@WebMvcTest` controllers and `@SpringBootTest` integration tests.

They need the **Config Server running**; without it the context fails and the whole suite fails with it. That is why the deploy hook has the test step commented out.

Spring Boot 4 moved the test slices: `@WebMvcTest` and `@AutoConfigureMockMvc` are in `org.springframework.boot.webmvc.test.autoconfigure`, `@DataJpaTest` in `org.springframework.boot.data.jpa.test.autoconfigure`, `@AutoConfigureTestDatabase` in `org.springframework.boot.jdbc.test.autoconfigure`. `@MockBean` is gone — use `@MockitoBean` from `org.springframework.test.context.bean.override.mockito`. The matching `spring-boot-*-test` modules are declared in `pom.xml`.
